...
public class NongDA {
    ...
    private static final Logger log = LoggerFactory.getLogger(NongDA.class);
    ...
    public List<CategoryClickCount> courseClickCount() throws Exception {
        ...
        List<CategoryClickCount> list = categoryClickCountDAO.query(DataUtils.getPreviousDateFormatted());
        ...
        for(CategoryClickCount model:list){
            ...
            log.info(model.getName().substring(9));
            if(name!=null){
                model.setName(name);
            }else{
                model.setName("其他");
